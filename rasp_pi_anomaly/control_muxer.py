...
def get_gse_and_sm_lists(exo_list, config: Type[config_util.ConfigurableConstants]):
    ...
    if config.TASK == config_util.Task.WALKING:
        for exo in exo_list:
            ...
            reel_in_controller_used = constants.ControllerUsed.SmoothReelInController.value
            swing_controller = controllers.StalkController(
                exo=exo, desired_slack=config.SWING_SLACK)
            swing_controller_used = constants.ControllerUsed.StalkController.value
            if config.STANCE_CONTROL_STYLE == config_util.StanceCtrlStyle.FOURPOINTSPLINE:
                reel_out_controller = controllers.SoftReelOutController(
                    exo=exo, desired_slack=config.SWING_SLACK)
                reel_out_controller_used = constants.ControllerUsed.SoftReelOutController.value
                ...
                stance_controller_used = constants.ControllerUsed.FourPointSplineController.value
                ...
                stance_controller_used = constants.ControllerUsed.SawickiWickiController.value
            elif config.STANCE_CONTROL_STYLE == config_util.StanceCtrlStyle.VIRTUALNEUROMUSCULARCONTROLLER:
                reel_out_controller = controllers.SoftReelOutController(
                    exo=exo, desired_slack=8000, force_timer_to_complete=True)
                reel_out_controller_used = constants.ControllerUsed.SoftReelOutController.value
                stance_controller = controllers.VirtualNeuroMuscularController(exo=exo, config=config)
                stance_controller_used = constants.ControllerUsed.VirtualNeuromusuclarController.value
            state_machine = state_machines.StanceSwingReeloutReelinStateMachine(exo=exo,
                                                                                ...
                                                                                reel_out_controller=reel_out_controller,reel_in_controller_used=reel_in_controller_used,
                                                                                swing_controller_used=swing_controller_used,
                                                                                reel_out_controller_used=reel_out_controller_used,
                                                                                stance_controller_used=stance_controller_used,
                                                                                config=config)
        ...
        gait_phase_estimator = gait_state_estimators.BilateralSlipDetectorFromSync(
            exo_1=exo_list[0], exo_2=exo_list[1], delay_ms=config.SLIP_DETECT_DELAY)
        ...
        for exo in exo_list:
            ...
            standing_controller_used = constants.ControllerUsed.GenericImpedanceController.value
            if config.STANCE_CONTROL_STYLE == config_util.StanceCtrlStyle.GENERICIMPEDANCE:
                ...
                slip_controller_used = constants.ControllerUsed.GenericImpedanceController.value
                ...
                slip_controller_used = constants.ControllerUsed.FourPointSplineController.value
                ...
                slip_controller = controllers.FourPointSplineController(
                    ...
                    use_gait_phase=False,
                    peak_hold_time=0.1)
                slip_controller_used = constants.ControllerUsed.FivePointSplineController.value
                slip_recovery_time = 0.99
            ...
            state_machine = state_machines.StandingPerturbationResponse(exo=exo,
                                                                        ...
                                                                        slip_recovery_time=slip_recovery_time, standing_controller_used=standing_controller_used,
                                                                        slip_controller_used=slip_controller_used)
            state_machine_list.append(state_machine)
        ...
        for exo in exo_list:
            ...
            state_machine = state_machines.StanceSwingReeloutReelinStateMachine(exo=exo,
                                                                                ...
                                                                                swing_only=config.SWING_ONLY, reel_in_controller_used=reel_in_controller_used,
                                                                                swing_controller_used=swing_controller_used,
                                                                                reel_out_controller_used=reel_out_controller_used,
                                                                                stance_controller_used=stance_controller_used,
                                                                                config=config)
            state_machine_list.append(state_machine)
    elif config.TASK == config_util.Task.ANOMALY:

        jetson_interface = dl_util_anomaly.JetsonInterface(server_ip=config.SERVER_IP, recv_port=config.PORT
                                                    )
        ...
        for exo in exo_list:
            ...
            gait_state_estimator = gait_state_estimators.DLANOMALYESTIMATOR(
                side=exo.side,
                data_container=exo.data,
                heel_strike_detector=heel_strike_detector,
                gait_phase_estimator=gait_phase_estimator,
                toe_off_detector=toe_off_detector,
                do_print_heel_strikes=config.PRINT_HS, jetson_interface=jetson_interface)
            gait_state_estimator_list.append(gait_state_estimator)
    ...
    print(f"gait_state_estimator_list: {gait_state_estimator_list}")
    print(f"state_machine_list: {state_machine_list}")

    return gait_state_estimator_list, state_machine_list

    # elif config.TASK == config_util.Task.WALKJOGDLGAITPHASE:
    #     jetson_interface = dl_util.JetsonInterface(server_ip=config.SERVER_IP, recv_port=config.PORT
    #                                                 )
    #     temp_var = 1
    #     for exo in exo_list:
    #         gait_state_estimator = gait_state_estimators.DLGAITSTATEESTIMATOR(
    #             side=exo.side, data_container=exo.data, jetson_interface=jetson_interface,
    #             do_filter_gait_phase=config.DO_FILTER_GAIT_PHASE)
    #         gait_state_estimator_list.append(gait_state_estimator)
    #         # Define State Machine
