...
#                  jetson_interface: Type[dl_util_anomaly.JetsonInterface] = None,
...
#         self.jetson_object.package_and_send_message(side=self.side, data_container=data)
#         self.jetson_object.grab_message_and_parse()
#         anomaly_info = self.jetson_object.get_most_recent_anomaly(side=self.side)
#         if anomaly_info is not None: 
#             flat_mean_anomaly  = anomaly_info
#             # print(flat_mean_anomaly)


#         else: return

#         data.dl_anomaly_mean_flat = flat_mean_anomaly
...
class GaitStateEstimator():
    ...
    def update_params_from_config(self, config: Type[config_util.ConfigurableConstants]):
        pass
# class DLGAITSTATEESTIMATOR():
#     def __init__(self,
#                  side: Type[constants.Side],
#                  data_container: Type[exoboot.Exo.DataContainer],
#                  jetson_interface: Type[dl_util.JetsonInterface],
...
#         self.stride_average_gait_state_estimatot = StrideAverageGaitPhaseEstimator
#         self.jetson_object = jetson_interface
#         print(
...
#             data_container=self.data, heel_strike_detector=heel_strike_detector,
#             gait_phase_estimator=gait_phase_estimator, toe_off_detector=toe_off_detector)
       
  
#     def detect(self):  #pavan
#         self.jetson_object.package_and_send_message(side=self.side, data_container=self.data)
#         self.jetson_object.grab_message_and_parse()
#         my_gait_phase_info = self.jetson_object.get_most_recent_gait_phase(side=self.side)
#         # print(my_gait_phase_info)
...
#         elif gait_phase > 1: gait_phase = 1
#         self.data.dl_stance_phase = gait_phase
#         self.data.dl_is_stance = is_stance
#         self.data.did_heel_strike = 0 # False
#         self.data.did_toe_off = 0 # False
...
#             self.data.did_heel_strike = 1 # True
#         if not is_stance and self.last_is_stance:
#             self.data.did_toe_off = 1 # True
...
class DLANOMALYESTIMATOR():
    ...
    def detect(self):
        data = self.data_container  # For convenience
        # Set up the LED on GPIO pin 18
        led_right = gpiozero.LED(18)
        led_left = gpiozero.LED(17)
        data.did_heel_strike = self.heel_strike_detector.detect(data)
        data.gait_phase = self.gait_phase_estimator.estimate(data)
        data.did_toe_off = self.toe_off_detector.detect(data)
        if self.do_print_heel_strikes and data.did_heel_strike:
            print('heel strike detected on side: %-*s  at time: %s' %
                  (10, self.side, data.loop_time))
        #getting 8 channels from pi to jetson
        self.jetson_object.package_and_send_message(side=self.side, data_container=data)
        #after getting prediction, make inference for each side, mae and each side
        self.jetson_object.grab_message_and_parse()
        #get most recent mae value
        anomaly_info = self.jetson_object.get_most_recent_anomaly(side=self.side)
        if anomaly_info is not None: 
            flat_mean_anomaly  = anomaly_info
            thresholded_output = np.where(flat_mean_anomaly > self.threshold_value, 0, 1)
            if (self.side == 'RIGHT'):
                if (thresholded_output ==1):
                    led_right.on()
                elif (thresholded_output ==0):
                    led_right.off()
            if (self.side == 'LEFT'):
                if (thresholded_output ==1):
                    led_left.on()
                    ...
                    led_left.off()
            print('thresholded output: ', thresholded_output, 'flat mean anomaly: ', flat_mean_anomaly, 'side: ', self.side)

        else: return

        data.dl_anomaly_flat_mean = flat_mean_anomaly
        data.dl_anomaly_thresholded = thresholded_output
    def update_params_from_config(self, config: Type[config_util.ConfigurableConstants]):
        pass
