        ...
        try:
            if IS_HARDWARE_CONNECTED:
                ...
                exo_list.append(Exo(IS_HARDWARE_CONNECTED, dev_id=int(port_numbers[count]), file_ID=file_ID,
                                    ...
                                    do_include_gen_vars=config.DO_INCLUDE_GEN_VARS,
                                    sync_detector=sync_detector,
                                    left_side_offline_data=offline_data_left,
                                    right_side_offline_data=offline_data_right))
                count += 1
        except IOError:
...
class Exo():
    def __init__(self,
                 ...
                 sync_detector=None,
                 left_side_offline_data=None,
                 right_side_offline_data=None):
        ...
        self.left_side_offline_data = left_side_offline_data
        self.right_side_offline_data = right_side_offline_data
        self.dev_id = dev_id
        ...
        if self.dev_id is None:
            ...
            if not self.IS_HARDWARE_CONNECTED: self.actpack_data = left_side_offline_data
            ...
            if not self.IS_HARDWARE_CONNECTED: self.actpack_data = right_side_offline_data
        else:
            raise ValueError(
    ...
    class DataContainer:
        ...
        did_heel_strike: float = 0 # bool = False
        gait_phase: float = None
        did_toe_off: float = 0 # bool = False
        ...
        dl_gait_phase: float = None
        dl_stance_phase: float = None
        dl_is_stance: float = None
        dl_velocity: float = None
        dl_ramp_angle: float = None
        dl_did_heel_strike: float = 0
        dl_did_toe_off: float = 0
        dl_anomaly_flat_mean: float = 0
        dl_anomaly_thresholded: float = 0
        # Additional Exoboot Parameters/Data
        commanded_voltage: int = None
        commanded_motor_impedance: float = None
        controller: str = None
    ...
    def read_data(self, exo_boot_side=None, iteration_count=None, loop_time=None):
        ...
        if self.IS_HARDWARE_CONNECTED:
            ...
            ank_ang = self.actpack_data['ankle_angle'][iteration_count]
            ankle_angle_temp = ank_ang
        ...
        if self.IS_HARDWARE_CONNECTED:
            ...
            if (self.last_state_time is None or last_ankle_angle is None or self.data.state_time-self.last_state_time > 20):
                self.data.ankle_velocity = 0
            elif self.data.state_time == self.last_state_time:
                pass  # Keep old velocity
            else:
                angular_velocity = (self.data.ankle_angle - last_ankle_angle)/(self.data.state_time-self.last_state_time)
                self.data.ankle_velocity = self.ankle_velocity_filter.filter(angular_velocity)
        else:
            if self.has_calibrated:
                self.data.slack = float(self.actpack_data['slack'][iteration_count])
            if self.last_state_time is None or last_ankle_angle is None or self.data.state_time - self.last_state_time > 20:
                self.data.ankle_velocity = 0
            elif self.data.state_time == self.last_state_time:
                pass  # Keep old velocity
            else:
                self.data.ankle_velocity = float(self.actpack_data['ankle_velocity'][iteration_count])
    ...
    def write_data(self, only_write_if_new: bool = True):
        ...
        if self.file_ID is not None and only_write_if_new:
            ...
            if self.last_state_time != self.data.state_time:
                ...
                self.data.did_heel_strike = 1 if self._did_heel_strike_hold else 0
                self.data.did_toe_off = 1 if self._did_toe_off_hold else 0
    ...
    def standing_calibration(self,
                             ...
                             max_seconds_to_calibrate: float = 20,
                             current_threshold: float = 1500,
                             do_zero_ankle_angle: bool = False): # 5
