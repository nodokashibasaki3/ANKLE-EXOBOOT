...
import math
from muscle_model import MusculoTendonJoint as MTJ
import numpy as np
...
class Controller(object):
    ...
    def update_muscle_model(self, m_stim, phi0):
        pass

class VirtualNeuroMuscularController(Controller):

    def __init__(self, exo: Exo, config,
                 Kp: int = 40,
                 Ki: int = 400,
                 Kd: int = 0,
                 ff: int =120):
        ...
        self.VNMC_GAIN = config.VNMC_GAIN
        self.mscl = None
        self.config = config
        self.vnmc_dict = {'mtu_force': [], 'length_CE': [], 'velocity_CE': [], 'vnmc_torque': [], 'm_stim': []}

        self.name_0, self.F_mtu_0, self.l_ce_0, self.v_ce_0 = 0, 0, 0, 0

        self.n_update_muscle = config.MUSCLE_UPDATE_FREQUENCY
        self.l_opt = 0.04     # [m] 0.1
        self.v_max = 6    # [lopt / s] 12
        self.l_slack = 0.26   # [m] 0.4
        self.rho = 0.5        # 1
        self.muscle_name = 'm0'
        self.feedback_data = 'F_mtu'  # can be 'F_mtu', 'l_ce', 'v_ce'
        ...
        mtu_par = {'F_max': self.F_max, 'l_opt': self.l_opt, 'v_max': self.v_max, 'l_slack': self.l_slack,
                   'rho': self.rho, 'name': self.muscle_name, 'r1': self.r0, 'phi1_ref': self.phi_ref, 'A': 0.01,
                   'flag_afferent': True, 'del_t': 0.02, 'phi1_0': math.radians(self.phi0)}
        self.muscles = {'m0': MTJ((1/config.TARGET_FREQ), mtu_par, n_update=self.n_update_muscle, FeedbackfromSensoryData=self.feedback_data)}
        self.mscl = self.muscles['m0']
    ...
    def command(self, reset):
        ...
        self.phi0 = self.exo.data.ankle_angle

        # Below 2 Lines are for VNMC with Positive Force Feedback
        F_mtu_0 = self.mscl.getSensoryData()
        m_stim = 0.01 + self.VNMC_GAIN * F_mtu_0

        self.update_muscle_model(m_stim, self.phi0)

        self.exo.command_torque(self.SCALING_FACTOR * self.trq)

    def update_muscle_model(self, m_stim, phi0):

        self.mscl.update(m_stim, math.radians(phi0), self.config, self.exo)

        self.name_0, self.F_mtu_0, self.l_ce_0, self.v_ce_0 = self.mscl.getStates()
        self.trq = self.mscl.getTorque()

        self.vnmc_dict['mtu_force'].append(self.F_mtu_0)
        self.vnmc_dict['length_CE'].append(self.l_ce_0)
        self.vnmc_dict['velocity_CE'].append(self.v_ce_0)
        self.vnmc_dict['vnmc_torque'].append(self.trq)
        self.vnmc_dict['m_stim'].append(m_stim)

    def update_ctrl_params_from_config(self, config: Type[config_util.ConfigurableConstants]):
        'Updates controller parameters from the config object.'''
        if self.SCALING_FACTOR != config.SCALING_FACTOR:
            self.SCALING_FACTOR = config.SCALING_FACTOR
            print('Scaling Factor updated to: ', self.SCALING_FACTOR)
        if self.VNMC_GAIN != config.VNMC_GAIN:
            self.VNMC_GAIN = config.VNMC_GAIN
            print('VNMC Gain updated to: ', self.VNMC_GAIN)
        if self.mscl.n_update != config.MUSCLE_UPDATE_FREQUENCY:
            self.mscl.n_update != config.MUSCLE_UPDATE_FREQUENCY
            print('Muscle Update Frequency updated to: ', self.mscl.n_update)
...
class SmoothReelInController(Controller):
    def __init__(self,
                 exo: Exo,
                 reel_in_mV: int = 1800, #1200
                 slack_cutoff: float = 1500,
                 time_out: float = 0.6,
                 Kp: int = 100,#30,  # 50  150
                 Ki: int = 10,#30,  # 10   50
                 Kd: int = 0,
                 ff: int = 0):
    ...
    def command(self, reset=False):
        ...
        self.exo.command_voltage(
            ...
            # print(self.reel_in_mV)
        # for ramp_up_value in np.arange(1, 0, -0.01):
        # #     # time.sleep(0.08)
        #      self.exo.command_voltage(desired_mV=ramp_up_value * self.exo.motor_sign * 1200)
        # self.exo.command_voltage(desired_mV=  self.exo.motor_sign * self.reel_in_mV)
        # self.exo.command_slack(desired_slack=200)

    def check_completion_status(self):
        # print(self.exo.get_slack())
        if self.delay_timer.check() or self.exo.get_slack() < self.slack_cutoff:

            # print(self.delay_timer.check())
            # print('-------------------------------------')
            # print(self.exo.get_slack() < self.slack_cutoff)
            # if self.delay_timer.check():
            #     # print('reel in timed out')
            # if self.exo.get_slack() < self.slack_cutoff:
            #     # print('slack cutoff reached')
...
class SoftReelOutController(Controller):
    ...
    def check_completion_status(self):
        slack = self.exo.get_slack()
        # print(slack, not self.force_timer_to_complete, slack > self.desired_slack-500, self.delay_timer.check())
