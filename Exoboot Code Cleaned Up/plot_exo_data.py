...
def print_options(is_vnmc_included_in_past_data, is_vnmc_included_in_new_data, is_controller_control_state_included_in_past_data):

    a, val_a = "1: Acceleration & Gyro Data\n", 1
    b, val_b = "2: Motor Angle, Velocity & Current data\n", 2
    c, val_c = "3: Ankle Angle, Velocity & Torque from Current Data\n", 3
    d, val_d = "4: Did Heel Strike (Boolean), Did Toe Off (Boolean) & gait_phase data\n", 4
    e, val_e = "5: Command Current, Position and Torque data\n", 5
    f, val_f = "6: Slack & Temperature data\n", 6
    if is_vnmc_included_in_past_data or is_vnmc_included_in_new_data:
        if is_controller_control_state_included_in_past_data:
            g, val_g = "7: Controller Implemented and Control State Data\n", 7
            h, val_h = "8: Muscle Tendon Unit (MTU) Length, Velocity, Torque, Force, Muscle Stimulation and Ankle Angle\n", 8
            print(a + b + c + d + e + f + g + h)
            list_of_vals = [val_a, val_b, val_c, val_d, val_e, val_f, val_g, val_h]
        else:
            h, val_h = "7: Muscle Tendon Unit (MTU) Length, Velocity, Torque, Force, Muscle Stimulation and Ankle Angle\n", 7
            val_g = 0
            print(a + b + c + d + e + f + h)
            list_of_vals = [val_a, val_b, val_c, val_d, val_e, val_f, val_g, val_h]
    else:
...
def plotter(new_data_file, past_data_file, past_data_include):
    ...
    is_controller_control_state_included_in_past_data = False
    if 'controller' in list(past_data_file.columns) and 'control_state' in list(past_data_file.columns):
        is_controller_control_state_included_in_past_data = True

    list_of_vals = print_options(is_vnmc_included_in_past_data, is_vnmc_included_in_new_data, is_controller_control_state_included_in_past_data)
    ...
    while loop_choice_of_plot:
        ...
        if choice_of_plot == list_of_vals[0]:
        ...
        elif choice_of_plot == list_of_vals[1]:
            if past_data_include:
        ...
        elif choice_of_plot == list_of_vals[2]:
            if past_data_include:
        ...
        elif choice_of_plot == list_of_vals[3]:
            if past_data_include:
        ...
        elif choice_of_plot == list_of_vals[4]:
            if past_data_include:
        ...
        elif choice_of_plot == list_of_vals[5]:
            if past_data_include:
        ...
        elif choice_of_plot == list_of_vals[6] and list_of_vals[6] != 0:
        ...
        elif (choice_of_plot == list_of_vals[7] and list_of_vals[7] != 0) or (choice_of_plot == list_of_vals[6] and list_of_vals[6] != 0):
            ...
            fig = make_subplots(rows=3, cols=2, subplot_titles=("VNMC Torque", "Velocity CE", "MTU Force", "Ankle Angle", "Length CE", "Muscle Stimulation"))
            ...
            if past_data_include and is_vnmc_included_in_past_data:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        past_data_yes_no = input('''Do you want to plot Past Data along with New Data [y/n]:''')
        print('')
        if past_data_yes_no.lower() == 'y':
            ...
            print('Input not understood. Type [y/n] + Enter.')
