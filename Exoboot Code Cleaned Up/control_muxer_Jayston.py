...
controlmuxer = config_util.ConfigurableConstants()
...
def get_gse_and_sm_lists(exo_list, config: Type[config_util.ConfigurableConstants]):
    ...
    if config.TASK == config_util.Task.WALKING:
        ...
        jetson_interface = dl_util.JetsonInterface(server_ip=config.SERVER_IP, recv_port=config.PORT,
                                                    do_gp_sp_isp_bilateral=config.DO_GP_SP_ISP_BILATERAL,
                                                    do_velocity_bilateral=config.DO_VELOCITY_BILATERAL,
                                                    do_ramp_bilateral=config.DO_RAMP_BILATERAL)
        temp_var = 1
        for exo in exo_list:
            gait_state_estimator = gait_state_estimators.DLGAITSTATEESTIMATOR(
                side=exo.side, side_data_container=exo.data, other_side_data_container=exo_list[temp_var].data, 
                jetson_interface=jetson_interface,
                do_filter_gait_phase=config.DO_FILTER_GAIT_PHASE,
                do_gp_sp_isp_bilateral=config.DO_GP_SP_ISP_BILATERAL,
                do_velocity_bilateral=config.DO_VELOCITY_BILATERAL,
                do_ramp_bilateral=config.DO_RAMP_BILATERAL)
